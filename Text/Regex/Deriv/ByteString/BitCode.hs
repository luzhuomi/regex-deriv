...
fuse :: Path -> SPath -> SPath
fuse b (SEps p) = SEps $! (b `appP` p)
fuse b (SL p)   = SL $! (b `appP` p)
fuse b (SChoice p [sp1,sp2]) = 
  let !bp = b `appP` p
  in SChoice bp  $! [sp1, sp2]
fuse b (SChoice p [sp]) = let !bp = b `appP` p in SChoice bp $! [sp]
fuse b (SPair p sp1 sp2) = let !bp = b `appP` p in SPair bp sp1  sp2
fuse b (SStar p sp) = let !bp = b `appP` p in SStar bp sp
fuse b SPhi = SPhi
...
simp (Choice [r1,r2] gf) 
  | r1 == r2 = (r1, \(SChoice !p [!sp1,!sp2]) -> let !p' = (p `appP` zeroP) in fuse p' sp1)
  | isPhi r1 = (r2, \(SChoice !p [!sp1,!sp2]) -> let !p' = (p `appP` oneP) in fuse p' sp2)
  | isPhi r2 = (r1, \(SChoice !p [!sp1,!sp2]) -> let !p' = (p `appP` zeroP) in fuse p' sp1)
...
simp (ChoiceInt [ChoiceInt [r1,r2], r3]) =    
  (ChoiceInt [r1, ChoiceInt [r2,r3]], \(SChoice !p1 [SChoice !p2 [!sp1, !sp2], !sp3]) ->
    let 
      ...
      !sp2' = fuse p12 sp2
      !sp3' = fuse p1 sp3
      !sp23'' = SChoice emptyP [sp2', sp3']
    in SChoice emptyP [sp1', sp23''])
simp (ChoiceInt [r1,r2]) 
  | r1 == r2 = (r1, \(SChoice !p [!sp1,!sp2]) -> fuse p sp1)
  | isPhi r1 = (r2, \(SChoice !p [!sp1,!sp2]) -> fuse p sp2)
  | isPhi r2 = (r1, \(SChoice !p [!sp1,!sp2]) -> fuse p sp1)
  | isJust (simpChoice r1 [] r2) = fromJust (simpChoice r1 [] r2)
  | otherwise = let (r1',!f1) = simp r1
                    (r2',!f2) = simp r2
                in r1' `seq` r2' `seq` (ChoiceInt [r1',r2'], \(SChoice !p [!sp1,!sp2]) -> 
       ...
       the proof terms coercing from  [| r1 +  r2 ... rn-1 + rn |] -> [| r1 +  r2 ... rn-1|]
         \x -> case x of 
           { SChoice p1 [sp1, [SChoice p2 [ ... SChoice pn-1 [spn-1, spn]]]] 
              -> SChoice p1 [sp1, [SChoice p2 [ ... pn-1+[0] `fuse` spn-1 ]]]
           }
    case 2)  otherwise 
      case rn = rn_a + rn_b
        case 2a) r1 == rn_a
         ...
         \x -> case x of 
            { SChoice p1 [sp1, [SChoice p2 [ ... SChoice pn [spn_a, spn_b]]]] 
              -> SChoice p1 [sp1, [SChoice p2 [ ... pn+[1] `fuse` spn_b]]]  
            }
        case 2b) otherwise 
...
fuseNthChoiceLeft :: Int -> SPath -> SPath
{-
fuseNthChoiceLeft 0 sp = sp
fuseNthChoiceLeft 1 (SChoice p [sp1,sp2]) = (p `appP` zeroP) `fuse` sp1
-}
fuseNthChoiceLeft 0 (SChoice !p [!sp1,!sp2]) = let !p' = p `appP` zeroP 
                                            in fuse p' sp1
fuseNthChoiceLeft n (SChoice !p [!sp1,!sp2]) = let !sp2' = fuseNthChoiceLeft (n-1) sp2  
                                               in SChoice p [sp1, sp2']

fuseNthChoiceRight :: Int -> SPath -> SPath
{- 
fuseNthChoiceRight 0 sp = sp
fuseNthChoiceRight 1 (SChoice p [sp1,sp2]) = (p `appP` oneP) `fuse` sp2
-}
fuseNthChoiceRight 0 (SChoice !p [!sp1,!sp2]) = let !p' = p `appP` oneP 
                                                in fuse p' sp2
fuseNthChoiceRight n (SChoice !p [!sp1,!sp2]) = let !sp2' = fuseNthChoiceRight (n-1) sp2 
                                                in SChoice p [sp1, sp2']
